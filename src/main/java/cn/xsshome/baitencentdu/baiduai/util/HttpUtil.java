package cn.xsshome.baitencentdu.baiduai.util;
...
public class HttpUtil {
    ...
    public static String postASR(String requestUrl, String params) throws Exception {
        ...
        out.close();
        // 建立实际的连接
        System.out.println("打开链接，开始发送请求"+dateFormat.format(new Date()));
        connection.connect();
//        // 获取所有响应头字段
//        Map<String, List<String>> headers = connection.getHeaderFields();
//        // 遍历所有的响应头字段
...
//        }
        ...
        if (requestUrl.contains("nlp")){
            ...
            in = new BufferedReader(new InputStreamReader(connection.getInputStream(), "UTF-8"));
            System.out.println("请求结束111"+dateFormat.format(new Date()));
        }
        String result = "";
        String getLine;
        System.out.println("请求结束"+dateFormat.format(new Date()));
        while ((getLine = in.readLine()) != null) {
            result += getLine;
        }
        in.close();
